  ...
  <div class="container flex flex-col items-center">
    <div class="flex flex-col items-center">
      ...
      <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-6"
        @click="state.count++">Calculate</button>
      <p class="text-2xl font-semibold text-lime-900 mt-2">{{ state.count }} times clicked</p>
    </div>

    <div class="flex flex-col items-center">
      <input type="text" placeholder="Search for a joke" v-mode="searchQuery" @input="searchForJokes"
...
const queryTimeout = reactive(null)
const jokesSearchResult = reactive(null)
...
const searchForJokes = async () => {
  ...
  queryTimeout.value = setTimeout(async () => {
    if (searchQuery.value !== "") {
      const result = await axios(`https://api.chucknorris.io/jokes/search?query=${searchQuery.value}`)
      jokesSearchResult.value = result.data.value
      console.log(jokesSearchResult.value)
    }
